...
class User {
    ...
    function __construct(string $n, string $fn, string $ip, string $browser ) {
        $this->usr_name = $n;
    ...
    public static function login(PDO $db,  $request,  $session) {
        ...
        $stmt = $db->prepare("SELECT userId, email, password, username, firstname, lastname, date, verified, admin FROM Users WHERE username=:username");
        ...
        if ($row = $stmt->fetch(PDO::FETCH_ASSOC))
        {
            if (password_verify($request->request->get('password'), $row['password']) && $row['verified'] == 1) {
                ...
                $session->set('User', new User($request->request->get('username'), $firstname . " " . $lastname, $ip, $browser));
                return true;
            }
        }
        else return false;
