...
(56, 'SomeTextFile.txt', 'text/plain', 'ghju', '2020-05-09', 'abc', '126', 1, 'donald', 7, 1, 0x446574746520657220656e20746573742066696c206d656420656e207465737420696e6e686f6c642e0d0a46696c656e2068617220696b6b65206e6f6520696e746572657373616e7420696e6e686f6c642e0d0a44656e6e652066696c656e206572206f7070726574746574206261726520666f722074657374696e672e);
...
('aka160@post.uit.no', '$2y$10$vQiNmB6g3shbyqMTyK5pM./Cmp/VKvr1O3RzJFkiHguNmbxt6wIKi', 'donald', 'Abdullah', 'Karagoez', '2020-05-05', '7ff0d1767def86c1b26a800e0aeccb18', 1, 1),
('abd.karagoz@gmail.com', '$2y$10$JlhWaTyxwvXcbeBmyKtTfucahctdbOcM8xbgogbZ7bbulbvgDbd2q', 'donaldd', 'Abdullah', 'Karagoez', '2020-05-12', '7839a8b74485830751ad1f0d6ed1ddc1', 1, 0),
('duongnvt94@hotmail.com', '$2y$10$JQBjTBtsA2av4VJoyXFfWudl5MGduSJAm3xE6EhqS/.pQPqqGPtxa', 'duong011', 'Duong', 'Nguyen', '2020-05-06', '804d11ff979963a19a4568ec1fca6c63', 1, 1),
...
CREATE ALGORITHM=UNDEFINED DEFINER=`stud_v20_karagoz`@`%` SQL SECURITY DEFINER VIEW `FilesWithTagsView`  AS  select `Files`.`fileId` AS `id`,`Files`.`title` AS `Title`,`Files`.`uploadedDate` AS `Date`,`Files`.`owner` AS `Author`,`Files`.`size` AS `Size`,`Files`.`filename` AS `Filename`,`Files`.`access` AS `Access`,`Files`.`type` AS `Type`,`Files`.`impressions` AS `Views`,`Files`.`catalogId` AS `Catalog`,1 AS `isFile`,`FilesAndTags`.`tag` AS `tag` from (`Files` join `FilesAndTags` on(`FilesAndTags`.`fileId` = `Files`.`fileId`)) where 1 ;
...
--
ALTER TABLE `Catalogs`
  MODIFY `catalogId` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=39;
